...
export default function PersonalDebtSummaryFormPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      const access = await checkUserTemplateAccess(user.id, 'personal_debt_summary');
    ...
    try {
      if (!submissionId) {
        const { data, error } = await supabase.from('template_submissions').insert({
          user_id: user.id, template_type: 'personal_debt_summary', form_data: form
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/generate-pdf', {
        ...
        body: JSON.stringify({ submissionId: submissionId, templateType: 'personal_debt_summary' })
      });
      if (!res.ok) {
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-8">
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center mb-4">
                  <div className="w-12 h-12 bg-gradient-to-br from-orange-500 to-red-600 rounded-xl flex items-center justify-center text-2xl mr-4">📋</div>
                  <div>
                    <h1 className="text-3xl font-bold text-gray-900">Personal Debt Summary</h1>
                    <p className="text-gray-600">Complete Overview of Personal Liabilities</p>
                  </div>
                </div>
                <p className="text-gray-600 max-w-2xl">
                  Organize all your personal debts in one clear document that lenders can quickly review and understand. 
                  This comprehensive summary helps demonstrate your debt management and repayment capacity.
                </p>
              </div>
              <div className="hidden lg:block">
                <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-2xl p-6 text-center">
                  <div className="text-2xl font-bold text-orange-600">${totalCurrentBalance.toLocaleString()}</div>
                  <div className="text-sm font-medium text-orange-600">Total Debt</div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <input type="text" placeholder="Enter your full legal name" className={`w-full border-2 rounded-xl px-4 py-3 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors ${
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">SSN (Optional)</label>
              <input type="text" placeholder="XXX-XX-XXXX" className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                value={form.personalInfo.ssn || ''} onChange={(e) => updateForm('personalInfo.ssn', e.target.value)} />
              <p className="text-xs text-gray-500 mt-1">For lender verification purposes</p>
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">As of Date *</label>
              <input type="date" className={`w-full border-2 rounded-xl px-4 py-3 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors ${
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={addDebt} className="px-4 py-2 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-xl hover:from-orange-600 hover:to-red-700 transition-all shadow-lg font-medium text-sm">
              + Add Debt
            </button>
          </div>
          ...
          {form.debts.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No Debts Added Yet</h3>
              <p className="text-gray-600 mb-4">Click "Add Debt" to start listing your personal liabilities</p>
              <button onClick={addDebt} className="px-6 py-3 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-xl hover:from-orange-600 hover:to-red-700 transition-all shadow-lg font-medium">
                Add Your First Debt
              </button>
            </div>
          ) : (
            <div className="space-y-6">
              {form.debts.map((debt, index) => (
                <div key={index} className="border-2 border-gray-200 rounded-xl p-6 relative">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Debt #{index + 1}</h3>
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Creditor Name *</label>
                      <input type="text" placeholder="Bank, Credit Card, etc." className="w-full border-2 border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                        value={debt.creditor} onChange={(e) => updateDebt(index, 'creditor', e.target.value)} />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Account Number</label>
                      <input type="text" placeholder="Last 4 digits: ****1234" className="w-full border-2 border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                    ...
                    <div>
                      ...
                      <input type="number" step="0.01" placeholder="$0.00" className="w-full border-2 border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                    ...
                    <div>
                      ...
                      <input type="number" step="0.01" placeholder="5.25" className="w-full border-2 border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                    ...
                    <div>
                      ...
                      <input type="date" className="w-full border-2 border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                    ...
                    <div className="lg:col-span-2">
                      ...
                      <input type="text" placeholder="House, car, or other secured asset" className="w-full border-2 border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
          ...
          <textarea className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors" rows={4}
            placeholder="Add any additional context about your debts, payment history, or other relevant information..."
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-gray-900">Debt Summary</h2>
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-orange-50 rounded-xl p-4 text-center">
              <div className="text-orange-600 font-semibold">Total Accounts</div>
              <div className="text-xl font-bold text-orange-700">{form.debts.length}</div>
            </div>
          </div>
          <div className="flex items-center justify-between pt-6 border-t">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button onClick={onGenerate} className="px-8 py-3 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-xl hover:from-orange-600 hover:to-red-700 transition-all shadow-lg font-semibold" disabled={loading}>
